<template>
	<view class="goods-details">
		...
		<view class="contain" v-if="!isNull">
			...
			<navigator class="mt20" hover-class="none" url="/bundle_order/pages/server_explan/server_explan?type=2">
			...
			<view class="evaluation bg-white mt20">
				<navigator hover-class="none" :url="'/bundle_activity/pages/all_comments/all_comments?id=' + goodsDetail.id"
			...
			<view class="footer row bg-white fixed">
				<navigator class="btn column-center" hover-class="none"
					url="/bundle_order/pages/contact_offical/contact_offical">
		...
		<share-popup v-model="showShareBtn"
			:share-id="id"
			pagePath="bundle_activity/pages/goods_details/goods_details"
			:type="1"
			:config="{
	...
	export default {
		...
		methods: {
			...
			onBuy(e) {
				...
				uni.navigateTo({
					url: '/bundle_order/pages/confirm_order/confirm_order?data=' + encodeURIComponent((JSON.stringify(params)))
				})
				console.log(1111)
			},
			onConfirm(e) {
			...
			async onAddCart(e) {
				...
				if (this.goodsType == 2) {
					...
					uni.navigateTo({
						url: '/bundle_order/pages/confirm_order/confirm_order?data=' + encodeURIComponent((JSON.stringify({
							goods
						})))
					})
					return
		...
		async onShareAppMessage() {
			...
			return {
				...
				path: '/bundle_activity/pages/goods_details/goods_details?id=' + this.id + "&invite_code=" + userInfo.distribution_code
			};
		},
		computed: {
